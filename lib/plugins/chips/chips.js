(function (window) {
	...
	class Chip {
		...
		getInput() {
			...
			let list = this.getPlugin().getList(),
				name = this.getPlugin().getContainer().getMatrixName();
	...
	class Chips extends Form.Plugin {
		...
		constructor(container) {
			...
			let form = this.getWrite(),
				plugin = form.addInput({
					name: 'write',
			...
			try {
				...
				if (matrix.patterns[0].hasOwnProperty('search') && typeof Form.Plugin.Chips.Search === 'function') {
					...
					let row = form.getRow(input.name),
						button = row.addButton('expand_more').getButton();

(function (window) {
    ...
    class Li {
        ...
        toggle(force) {
            ...
            input.checked = typeof force === 'boolean'
                ? force
                : !input.checked;
    ...
    class Checkbox extends Form.Plugin {
        ...
        arrange(value) {
            ...
            for (let item = 0; item < value.length; item++) value[item] = value[item].toString();
            for (let item = 0, list = this.getList(); item < list.length; item++)

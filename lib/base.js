(function (window) {
	...
	class Plugin {
		...
		dispatch() {
			...
			for (let item = 0; item < events.length; item++) {
				...
				event.initEvent(events[item], true, false);
	...
	class Text extends Plugin {
		...
		constructor(container) {
			...
			this.elements.preloader = new Preloader(this, this.getContent());
		}
	...
	class Container {
		...
		getMatrixDescription() {
			let matrix = this.getMatrix();
			if (matrix === null
				|| !matrix.hasOwnProperty('description')) return null;
		...
		getMatrixTooltip() {
			let matrix = this.getMatrix();
			if (matrix === null
				|| !matrix.hasOwnProperty('tooltip')) return null;

			return matrix.tooltip;
		}
		getMatrixRow() {
			let matrix = this.getMatrix();
			if (matrix === null
		...
		getMatrixEditable() {
			...
			return false === matrix.hasOwnProperty('editable')
				|| matrix.editable
		}
		getTooltipError() {
	...
	class Form extends Common {
		...
		request(everywhere_function) {
			...
			let data = new FormData(), values = this.get();
		...
		addInput(matrix, nofollow) {
			...
			try {
				...
				let container = new Container(this, matrix), plugin = container.getPlugin();
